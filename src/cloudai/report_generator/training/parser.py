...
class TrainingParser(ABC):
    ...
    def _get_from_dict(source: dict, path: str) -> Any:
        """Read a dotted path; a '*' segment globs that key against the current dict's keys."""
        value: Any = source
        for part in path.split("."):
            if not isinstance(value, dict):
                return None
            value = next((value[k] for k in fnmatch.filter(value, part)), None) if "*" in part else value.get(part)
        return value
